...
met_avg<-met[,.(temp=mean(temp,na.rm=TRUE), rh=mean(rh,na.rm=TRUE),wind.sp=mean(wind.sp,na.rm=TRUE),                    vis.dist=mean(vis.dist,na.rm=TRUE), lat=mean(lat), lon=mean(lon), dew.point = mean(dew.point, na.rm=TRUE), elev=mean(elev,na.rm=TRUE)), by="USAFID"]
...
```
- The association between dew point and relative humidity is possitive in all regions,the north west is the steepest.
...
```
for the dew point and relative humidity  the south east region has the highest mean value.
```{r}
met_avg %>%
  filter(!(region %in% NA)) %>%
...
rh_pal = colorNumeric(c('blue','purple','red'), domain=met_avg$rh)
...
```
from the map we can see that the east coast has highest relative humidity across the country, the relative humidity is decesing from east to west. However, the west coast area's relative humidity is almost as high as the east coast.
